...
public class H2TradeRepository implements TradeRepository {
	private Logger logger = Logger.getLogger(H2TradeRepository.class.getName());
	...
	private void initDB() {
		h2Client.inTransaction(session -> session
			.execute(INIT_DB)
			.doOnNext(i -> logger.info("DB SCHEMA INITIALIZED"))
		).blockLast();
	}
	...
	private void pingDB() {
		h2Client.withHandle(t -> t
			...
			.doOnNext(e -> logger.info("RESULT FOR SELECT 6 QUERY: " + e))
			.subscribe();
	}

	public Mono<Void> saveAll(Flux<Trade> trades) {
		return trades
			.doOnNext(t -> logger.info("[SQL] Requested to save trade: " + t))
			.flatMap(this::save)
			.then();
	}
	...
	public Mono<Void> save(Trade trade) {
		return h2Client.inTransaction(session ->
			session
				...
				.doOnNext(i -> logger.info("[SQL] Inserted rows: " + i))
		).then();
	}
	...
	public Mono<Void> showRepoAnalytics() {
		return h2Client.withHandle(handle -> handle
				...
				.reduce((s1, s2) -> s1 + ", " + s2)
				.doOnNext(e -> logger.info("[SQL] [*] Stored trades: " + e)
		).then();
	}
